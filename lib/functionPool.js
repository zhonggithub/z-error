...
**/
exports.verify = (data, mandatory, itemFuns, missingPrefix="MISSING", invalidPrefix="INVALIDE") => {
	let error = null;
	if (mandatory) {
		mandatory.some((item) => {
			if (!{}.hasOwnProperty.call(data, item)){
				const code = `${missingPrefix}_PARAMS`;
				const message = getMessage(`${missingPrefix}_${toLine(item)}`, config.lang || 'en') || getMessage(code, config.lang || 'en');
	...
	for (const item in itemFuns) {
		if ({}.hasOwnProperty.call(data, item)) {
			if (itemFuns[item](data[item]) === false) {
				const code = `${invalidPrefix}_PARAMS`;
				const message = getMessage(`${invalidPrefix}_${toLine(item)}`, config.lang || 'en') || getMessage(code, config.lang || 'en');
