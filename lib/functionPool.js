	...
	if (mandatory) {
		mandatory.some((item) => {
			if (!{}.hasOwnProperty.call(data, item)){
				const code = `${missingParamsPrefix}_${item.toUpperCase()}`;
				const message = getMessage(code, config.lang || 'en');
				const description = `Can\'t find the ${item} of ${JSON.stringify(data)}`;
				error = new ZError('Error', code, config.lang || 'en', message || code, description, 400);
				return true;
			}
		});
	}
	...
	for (const item in itemFuns) {
		if ({}.hasOwnProperty.call(data, item)) {
			if (itemFuns[item](data[item]) === false) {
				const code = `${invalideParams}_${item.toUpperCase()}`;
				const message = getMessage(code, config.lang || 'en');
				const description = `invalid ${data[item]} of ${item}`;
				error = new ZError('Error', code, config.lang || 'en', message || code, description, 422);
				return error;
			}
		}
	}
